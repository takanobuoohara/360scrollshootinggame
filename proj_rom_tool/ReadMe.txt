========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : proj_rom
========================================================================


AppWizard が生成した proj_rom には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには proj_rom を構成している各ファイルの概要説明が含まれています。
...
proj_rom.h
    ...
    みます） をインクルードし、また CProj_romApp アプリケーションクラスを宣
    言します。

proj_rom.cpp
    このファイルは CProj_romApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\proj_rom.ico
    ...
    ルです。このアイコンはリソース ファイル proj_rom.rc によってインクルード
    されます。
...
proj_romDoc.h, proj_romDoc.cpp - ドキュメント
    これらのファイルは CProj_romDoc クラスを含みます。特別なドキュメント デ
    ータを付け加えたり、CProj_romDoc::Serialize を使ったファイルのセーブや
    ロード機能をインプリメントしたいときはこのファイルを編集してください。

proj_romView.h, proj_romView.cpp - ビュー
    これらのファイルは CProj_romView クラスを含みます。
    CProj_romView オブジェクトは CProj_romDoc オブジェクトを表示するために
    使います。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) proj_rom.pch 
